...
function chooseYear(year) {
    const { shadowValue, onShadowValueChange, onChangeMode } = this.props
    const newValue = shadowValue.clone().year(year)
    // value.month(this.state.value.month())
    onShadowValueChange(newValue)
    onChangeMode('day')
}
...
class YearPanel extends Component {
        ...
        return years.map((row, index) => {
            const tds = row.map(yearData => {
                ...
                if (yearData.year < startYear) {
                    clickHandler = onChangeToPreviousDecade
                } else if (yearData.year > endYear) {
                    clickHandler = onChangeToNextDecade
                } else {
                ...
                }}><a className={`${this.prefixCls}-year`}> {yearData.content} </a> </td>)
            })
    ...
    render() {
        ...
        const yearsContent = years(shadowValue)
        const prefixCls = this.prefixCls

        const yearEls = this.getElements(yearsContent)
        return (
            <div className={`${prefixCls}-body`}>
                <table className={`${prefixCls}-table`} cellSpacing="0" role="grid">
                    <tbody className={`${prefixCls}-tbody`}>{yearEls}</tbody>
                </table>
            </div>
        )
    //   return (
...
function years(value) {
    ...
    range(ROW).forEach((rowIndex)=> {
        ...
        range(COL).forEach((colIndex) => {
            const index = rowIndex * COL  + colIndex
